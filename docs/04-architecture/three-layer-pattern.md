...
```
Test orchestrates → Helper uses → Config provides
```

**When you write a test:**

1. Test orchestrates: `await authHelper.login(...)`
2. Helper uses Config: `await this.page.fill(AUTH_UI.email, email)`
3. Config provides selectors: `email: '[data-testid="login-email"]'`
...
**Key difference:** Helpers encapsulate **business actions** (login, create, delete), not page mechanics (fillEmail, clickSubmit).
