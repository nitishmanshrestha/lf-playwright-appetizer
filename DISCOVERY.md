...
page.getByTestId("username")  // Most semantic
// OR
page.locator("input[name='user-name']")  // Less semantic
// Pick the first one (getByTestId)
```
...
- ✅ Fall back to getByRole or getByLabel if data-testid missing
...
| **No data-testid attributes** | Fallback to getByRole, getByLabel, getByText. Note in discovery doc |
