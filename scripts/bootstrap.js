...
const bootstrapEvidenceDir = path.join(
  rootDir,
  "playwright",
  "evidence",
  "bootstrap",
...
if (!existsSync(envFile)) {
  if (existsSync(envTemplate)) {
    ...
    console.log(
      "[bootstrap] Created .env from playwright/environments/.env.qa.example",
    );
    evidence.steps.envFile = "created";
  } else {
    console.warn(
      "[bootstrap] Warning: .env.qa.example not found. Skipping .env creation.",
    );
...
console.log("  2) Run npm run context:codegen to record your first flow");
console.log(
  "  3) Run npm run scaffold:flow -- --module <m> --feature <f> to scaffold tests",
);
console.log("  4) Start MCP from VS Code command palette: MCP: List Servers");
