...
function hasAssertionKeys(dataset) {
  ...
  return Object.keys(dataset).some(
    (key) => key.startsWith("expected") || key.startsWith("should"),
  );
}
...
try {
  ...
  const allErrors = [];
  for (const filePath of files) allErrors.push(...validateFixture(filePath));
  ...
  console.log(
    `[check:ddt-fixtures] Validated ${files.length} JSON fixture file(s).`,
  );
} catch (error) {
